...
echo "<style>body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,sans-serif;margin:2rem;color:#111}table{border-collapse:collapse;width:100%;max-width:1600px}th,td{padding:.4rem .6rem;border-bottom:1px solid #ddd;text-align:center}th{background:#f6f6f6;font-size:.85rem}td{font-family:monospace;font-size:.9rem}.legend{margin:1.5rem 0;padding:1rem;background:#f9f9f9;border-left:3px solid #666;max-width:1600px}.legend h3{margin:0 0 .5rem;font-size:1rem}.legend ol{margin:.5rem 0;padding-left:1.5rem;line-height:1.6;font-size:.9rem}.health{margin:1rem 0;padding:.6rem 1rem;border-radius:4px;font-size:.9rem;max-width:1600px}.health.ok{background:#d4edda;border:1px solid #c3e6cb;color:#155724}.health.warn{background:#fff3cd;border:1px solid #ffeaa7;color:#856404}.health.error{background:#f8d7da;border:1px solid #f5c6cb;color:#721c24}.health-dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:6px}.health-dot.ok{background:#28a745}.health-dot.warn{background:#ffc107}.health-dot.error{background:#dc3545}.health-failures{margin-top:.5rem;padding-left:1.2rem;font-size:.85rem;line-height:1.5}</style>";
echo "</head><body>";
echo "<div id=\"health-status\" class=\"health ok\"><span class=\"health-dot ok\"></span><strong>System OK</strong> <span style=\"color:#666;font-size:.8rem\">(checking...)</span></div>";
echo "<script>function updateHealth(){fetch('/api/health.php').then(r=>r.json()).then(d=>{const h=document.getElementById('health-status');const s=d.overall_status;h.className='health '+s;let dot='<span class=\"health-dot '+s+'\"></span>';let txt=s==='ok'?'System OK':s==='warn'?'System WARNING':'System ERROR';let time=' <span style=\"color:#666;font-size:.8rem\">('+new Date(d.checked_at).toLocaleTimeString()+')</span>';let fails='';if(d.failures.length>0){fails='<div class=\"health-failures\">';d.failures.forEach(f=>{fails+='<div>• '+f.message+'</div>';});fails+='</div>';}h.innerHTML=dot+'<strong>'+txt+'</strong>'+time+fails;}).catch(e=>{console.error('Health check failed:',e);});}updateHealth();setInterval(updateHealth,60000);</script>";
...
echo "<p><a href=\"/forecast.php\">PV Forecast vs Actual (mobile view)</a><br><a href=\"/home-consumption.php\">Home Consumption Actual</a></p>";
...
echo "<li>Forecast Solar Radiation (W/m²)</li>";
...
if (!$rows) {
  ...
  foreach ($rows as $row) {
    ...
    $fcRadText = $fcRad === null ? '-' : number_format((float)$fcRad, 1, '.', '');
