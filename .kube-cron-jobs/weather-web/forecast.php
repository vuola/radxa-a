...
$stmt = $pdo->prepare(
  "WITH slots AS (
   ...
   best_forecast AS (
     ...
     WHERE fv.target = 'pv_feed_in_w'
       AND fr.target = 'pv_feed_in_w'
   ...
   LEFT JOIN weather_fusion wf ON wf.ts = s.target_ts
...
$latestIssuedAt = null;
foreach ($rows as $row) {
  if ($row['yhat_p50'] !== null) {
    $forecastPointCount++;
  }
  if ($row['run_id'] !== null) {
    $runIdSet[(string)$row['run_id']] = true;
  }
  if ($row['model_name'] !== null && $row['model_version'] !== null) {
    $modelSet[(string)$row['model_name'] . ' ' . (string)$row['model_version']] = true;
  }
  if ($row['issued_at'] !== null) {
    $ts = (new DateTimeImmutable((string)$row['issued_at']))->getTimestamp();
...
$modelText = count($modelSet) > 0 ? implode(', ', array_keys($modelSet)) : ((string)$latestRun['model_name'] . ' ' . (string)$latestRun['model_version']);
$issuedAtText = $latestIssuedAt !== null
  ? (new DateTimeImmutable('@' . (string)$latestIssuedAt))->setTimezone($tz)->format('Y-m-d H:i')
  : (new DateTimeImmutable((string)$latestRun['issued_at']))->setTimezone($tz)->format('Y-m-d H:i');

echo "<p class=\"meta\">Model(s): <strong>" . htmlspecialchars($modelText) . "</strong> | Runs used: " . count($runIdSet) . " | Newest issue time: " . htmlspecialchars($issuedAtText) . "</p>";
